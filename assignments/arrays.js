...
const lastCarInfo = () => {
  return `The last car in the inventory is a ${
...
let modelList = inventory.map(item => item.car_model);

let alphabeticalModels = modelList.sort();

console.log(alphabeticalModels);
...
console.log(oldCarCount());

// ==== STRETCH ANSWER ====

let oldOnes = carYears.filter(item => item < 2000);

console.log(oldOnes.length);

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
let BMWAndAudi = [];

const onlyBMWAndAudi = () => {
  for (let i = 0; i < inventory.length; i++) {
...
const answer = JSON.stringify(onlyBMWAndAudi());
console.log(answer);

// ==== STRETCH ANSWER ====

let BMWnAudi = inventory.filter(
  item => item.car_make == "Audi" || item.car_make == "BMW"
);

console.log(JSON.stringify(BMWnAudi));
